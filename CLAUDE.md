...
Three current notebooks, no numeric ordering — pick based on what you need. (One more,
`RealCause_benchmark.ipynb`, still exists but is legacy — see "Notebook consolidation
in progress" below.)
...
- Uses `causal_bench` wrappers, like the legacy `RealCause_benchmark.ipynb` below (unlike quickstart/sandbox, which call each library's native API directly)
...
### Notebook consolidation in progress

This repo used to have three separate Lalonde-family notebooks: `Lalonde_benchmark.ipynb`
(real NBER data, no CATE ground truth possible — see `docs/LALONDE_DATASET.md`),
`RealCause_benchmark.ipynb` (RealCause semi-synthetic, no HPO), and
`RealCause_with_hpo_benchmark.ipynb` (RealCause semi-synthetic, with HPO — see above).
`Lalonde_benchmark.ipynb` has been deleted: this survey is about CATE accuracy, and real
Lalonde data can never supply that ground truth no matter the methodology, so a notebook
that could only ever check ATE wasn't worth maintaining in parallel — `docs/LALONDE_DATASET.md`
explains why in full, and `causal_bench.load_lalonde()` itself is unchanged if you want to
reproduce that comparison yourself. `RealCause_benchmark.ipynb` (no HPO) is still around, but
superseded by `RealCause_with_hpo_benchmark.ipynb` above — treat it as frozen/legacy too, and
expect it to be deleted in a future change. `scripts/build_new_notebooks.py`, which used to
generate the two legacy Lalonde/RealCause notebooks from a shared Python builder, has also
been deleted — notebooks are hand-maintained now, the same way the quickstart and sandbox
notebooks always were.
...
- Hand-maintained, like every notebook in this repo now (see "Notebook consolidation in progress" above)
...
- Supersedes `RealCause_benchmark.ipynb` below (and the now-deleted `Lalonde_benchmark.ipynb`) — new work happens here
...
**`RealCause_benchmark.ipynb`** — Legacy, pending removal
- Same 9 models (3 foundation + 6 metalearners) on RealCause semi-synthetic realizations, but no HPO — `RealCause_with_hpo_benchmark.ipynb` above is its direct successor.
- Still works and is still in the repo, but per "Notebook consolidation in progress" above, treat it as frozen — don't add features here, and don't be surprised when it's deleted in a future change. `Lalonde_benchmark.ipynb` (real NBER data, no HPO) used to be listed here too; it's already been deleted — see `docs/LALONDE_DATASET.md`.
